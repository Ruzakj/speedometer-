...
class SpeedometerApp : Application() {
    ...
    override fun onCreate() {
        ...
        registerActivityLifecycleCallbacks(object : ActivityLifecycleCallbacks {
            override fun onActivityCreated(activity: Activity, state: Bundle?) { if (activity is MainActivityV2) installHistoryButton(activity) }
            override fun onActivityResumed(activity: Activity) { if (activity is MainActivityV2) installHistoryButton(activity) }
            override fun onActivityPaused(activity: Activity) { if (activity is MainActivityV2) saveActiveTrip(activity) }
    ...
    private fun saveActiveTrip(activity: Activity) {
        runCatching {
            val c = activity.javaClass
            val tracking = c.getDeclaredField("tracking").apply { isAccessible = true }.getBoolean(activity)
            if (!tracking) return
