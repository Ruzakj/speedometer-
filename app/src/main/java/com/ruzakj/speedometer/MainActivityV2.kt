...
class MainActivityV2 : Activity(), LocationListener {
    ...
    override fun onCreate(state:Bundle?){super.onCreate(state);window.addFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);window.statusBarColor=BG;window.navigationBarColor=BG
        ...
        setContentView(dash);handler.post(refresh);playStartupSweep()
    }
    ...
    private inner class Dashboard(context:Context):View(context){
        ...
        override fun onDraw(c:Canvas){super.onDraw(c);c.drawColor(BG);if(pipMode)drawPip(c)else{val land=width>height;val p=transition.coerceIn(0f,1f);c.save();c.translate(0f,(1f-p)*df(12f));c.scale(.985f+.015f*p,.985f+.015f*p,width/2f,height/2f);if(land)drawLandscape(c)else drawPortrait(c);c.restore()}}
