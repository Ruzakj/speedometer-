...
class HistoryActivity : Activity() {
    ...
    override fun onCreate(state: Bundle?) {
        ...
        val root = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setPadding(dp(18), dp(18), dp(18), dp(18)); setBackgroundColor(bg) }
    ...
    private fun render(root: LinearLayout) {
        ...
        trips.forEachIndexed { index, trip ->
            val card = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setPadding(dp(16), dp(13), dp(16), dp(13)); background = rounded(if (index == 0) 0x2020E6FF else 0x181B2230) }
            val top = LinearLayout(this).apply { gravity = Gravity.CENTER_VERTICAL }
            top.addView(TextView(this).apply { text = "RIDE ${trips.size - index}"; textSize = 12f; setTextColor(if (index == 0) cyan else text); setTypeface(typeface, 1) }, LinearLayout.LayoutParams(0, dp(30), 1f))
            top.addView(TextView(this).apply { text = HistoryStore.date(trip.timestamp); textSize = 10f; setTextColor(muted) })
            card.addView(top)
            card.addView(TextView(this).apply { text = String.format(Locale.US, "%.2f km    •    MAX %.1f km/h    •    AVG %.1f km/h\nMOVING %s", trip.distanceKm, trip.maxKmh, trip.avgKmh, HistoryStore.duration(trip.movingMs)); textSize = 12f; setTextColor(text); setPadding(0, dp(4), 0, 0) })
            root.addView(card, LinearLayout.LayoutParams(-1, dp(88)).apply { setMargins(0, 0, 0, dp(10)) })
        }
    }

    private fun rounded(color: Int) = GradientDrawable().apply { setColor(color); cornerRadius = dp(18).toFloat(); setStroke(dp(1), 0x443B4A65) }
