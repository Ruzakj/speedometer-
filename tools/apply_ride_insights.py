...
insert = needle + '; private var smartMoving=false; private var stoppedSinceNs=0L; private var overspeedLimitKmh=80f; private val speedHistory=java.util.ArrayDeque<Float>()'
if needle not in text: raise SystemExit('state anchor not found')
...
new = '''locationManager=getSystemService(Context.LOCATION_SERVICE) as LocationManager;dash=Dashboard(this)
        ...
        val insights=RideInsightsOverlay(this,{speedKmh},{accuracyM},{speedAccuracyMps},{smartMoving},{overspeedLimitKmh},{setOverspeedLimit(it)},{speedHistory.toList()})
...
if old not in text: raise SystemExit('onCreate anchor not found')
...
if start < 0 or end < 0: raise SystemExit('location callback anchors not found')
...
if old_stop not in text: raise SystemExit('stopTracking anchor not found')
...
if old_reset not in text: raise SystemExit('resetTrip anchor not found')
...
'''
if anchor not in text: raise SystemExit('PIP anchor not found')
